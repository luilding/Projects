...
class GTFSUtils:
    @staticmethod
    def load_data(url: str, dataset_path: str) -> Optional[
        ...
        :return: A tuple containing the train stops DataFrame, stop_times DataFrame, routes DataFrame, 
                trips DataFrame, and calendar DataFrame.
        """
        ...
        try:
            with zipfile.ZipFile(BytesIO(response.content)) as z:
                ...
                for name in z.namelist():
                    ...
                    if name in ['2/google_transit.zip', '3/google_transit.zip']:
                        transit_type = 'train' if name == '2/google_transit.zip' else 'bus'
                        transit_path = os.path.join(dataset_path, transit_type)
                        os.makedirs(transit_path, exist_ok=True)
                        
                        logger.info(f"Found '{name}' inside the main zip file.")
                        # Extract the inner zip file
                        with z.open(name) as inner_zip_file:
                            with zipfile.ZipFile(BytesIO(inner_zip_file.read())) as inner_z:
                                logger.info(f"Extracting contents to '{transit_path}'...")
                                inner_z.extractall(transit_path)
                                logger.info(f"Extraction complete. Files saved to '{transit_path}'")
                                
                                
                                if transit_type == 'train':
                                    try:
                                        stops_df = pd.read_csv(os.path.join(transit_path, 'stops.txt'))
                                        routes_df = pd.read_csv(os.path.join(transit_path, 'routes.txt'))
                                        trips_df = pd.read_csv(os.path.join(transit_path, 'trips.txt'))
                                        stop_times_df = pd.read_csv(os.path.join(transit_path, 'stop_times.txt'))
                                        calendar_df = pd.read_csv(os.path.join(transit_path, 'calendar.txt'))
                                ...
                                # If this is bus data, just load and normalize it for future use
                                else:
                                    try:
                                        bus_stops_df = pd.read_csv(os.path.join(transit_path, 'stops.txt'))
                                        bus_stop_times_df = pd.read_csv(os.path.join(transit_path, 'stop_times.txt'))
                                        GTFSUtils.normalise_gtfs_data(bus_stops_df, bus_stop_times_df)
                                    except Exception as e:
                                        logger.info(f"Note: Bus data processing failed: {e}")

                if '2/google_transit.zip' not in z.namelist():
                    logger.error("No train data found in the zip file.")
                    return None
...
#             # Find route using the router
#             result = self.router.find_route(location_from, location_to)
...
#                 # Only send map link if we have a map file
#                 if map_file:
#                     current_dir = os.path.dirname(os.path.abspath(__file__))
...
#                     dispatcher.utter_message(text=f"I've generated a route map for you: {map_link}")
#             else:
#                 dispatcher.utter_message(text=f"No direct route found between {location_from} and {location_to}.")
...
# Add the current actions folder to Python's module search path
current_dir = os.path.dirname(os.path.abspath(__file__))
...
class ActionRunDirectionScript(Action):
    ...
    def _initialize_router(self):
        ...
        if self.router is None:
            try:
                # Change working directory to the actions folder to make relative paths work
                current_dir = os.path.dirname(os.path.abspath(__file__))
                os.chdir(current_dir)
                self.router = MultimodalTransitRouter()
                logger.info("Transit router initialized successfully")
            except Exception as e:
                logger.error(f"Failed to initialize transit router: {e}")
                raise
    ...
    def run(self, dispatcher: CollectingDispatcher,
        ...
        logger.info(f"Processing route request from {location_from} to {location_to}")
        
        if not location_from or not location_to:
            dispatcher.utter_message(text="Please provide both starting location and destination.")
            return []
        ...
        try:
            ...
            if isinstance(result, tuple):
                ...
                # Generate relative URL for the map using a dynamic relative path
                if map_file and os.path.exists(map_file):
                    # Use the same logic as the first implementation
                    relative_path = os.path.relpath(map_file, current_dir)
                    map_url = f"http://localhost:8000/{relative_path.replace(os.sep, '/')}"
                    map_link = f"<a href='{map_url}' target='_blank'>Click here to view the route map</a>"
                    dispatcher.utter_message(text=f"I've generated a route map for you: {map_link}")
                ...
                dispatcher.utter_message(text=result)  # This will be the error message
