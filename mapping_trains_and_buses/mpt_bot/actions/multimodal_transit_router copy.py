...
class MultimodalTransitRouter:
    ...
    def load_transit_data(self):
        ...
        # Update paths to look in the project root directory
        current_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(current_dir)  # Go up one level from actions/
        bus_dir = os.path.join(project_root, 'mpt_data', 'bus')
        train_dir = os.path.join(project_root, 'mpt_data', 'train')
        
        print("\nLoading GTFS files from:")
    ...
    def _load_or_build_shape_cache(self):
        ...
        cache_dir = os.path.join(project_root, 'mpt_data', 'cache')
    ...
    def _find_multimodal_route(self, start_stops, end_stops):
        ...
        print(f"Queue initialized with {len(queue)} start stops")
        
        iteration = 0
        while queue and iteration < 1000:  # Added iteration limit for safety
            iteration += 1
            current_stop_id, path, transfers, total_dist = queue.popleft()
            
            print(f"\nIteration {iteration}")
            current_stop = self.stops[self.stops['stop_id'] == current_stop_id].iloc[0]
            print(f"Current stop: {current_stop['stop_name']}")
            ...
            print(f"Found {len(routes)} routes from current stop")
            ...
            for next_stop, walk_dist in nearby:
                if next_stop['stop_id'] not in visited and next_stop['mode'] != current_stop['mode']:
                    ...
                    queue.append((next_stop['stop_id'], path + [next_stop_id], new_transfers, total_dist + walk_dist))
        
        print("No route found after search")
        return None, None

    def find_route(self, start_location, end_location):
        """Find and visualize a route between two locations"""
        print(f"\nSearching for route from {start_location} to {end_location}")
        ...
        print(f"\nNearest stops found:")
        print(f"Start stops: {[stop[0]['stop_name'] for stop in start_stops]}")
        print(f"End stops: {[stop[0]['stop_name'] for stop in end_stops]}")
        ...
        try:
            ...
            transfers = [{
                ...
                'description': f'Walk to {first_stop["stop_name"]} ({start_location})'
            }] + transfers + [{
                'type': 'walking',
                ...
                'description': f'Walk from {last_stop["stop_name"]} ({end_location}) to destination'
            }]
        ...
        for transfer in transfers:
            if transfer['type'] == 'walking':
                ...
                directions.append(
                    f"Take {transfer['mode']} {transfer['route']} to "
                    f"{self.stops[self.stops['stop_id'] == transfer['to_stop']].iloc[0]['stop_name']}"
                )
                total_transfers += 1
    ...
    def visualize_route(self, start_coords, end_coords, path, transfers):
        ...
        for transfer in transfers:
            if transfer['type'] == 'walking':
                try:
                    ...
                    print(f"Warning: Could not get walking route: {e}")
                    # Draw straight line if walking route fails
                    folium.PolyLine(
                ...
                try:
                    ...
                    print(f"Warning: Could not get route segment: {e}")
                    # Draw straight line if route segment fails
                    folium.PolyLine(
                        locations=[[transfer['from_lat'], transfer['from_lon']], 
                                [transfer['to_lat'], transfer['to_lon']]],
                        weight=4,
        ...
        output_dir = os.path.join(os.path.dirname(current_dir), 'mpt_data', 'maps')
